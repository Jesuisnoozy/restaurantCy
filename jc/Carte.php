<?php
// Nettoyage de l'espace invisible entre $data et =
$data = json_decode(file_get_contents("data/PMC.json"), true);
$plats = $data["plats"] ?? []; // Le ?? [] évite un crash si le fichier est vide
?>
<!DOCTYPE html>
<html lang="fr">
<head>
...
<main class="presentation">
    ...
    <section class="barre-de-recherche">
        <input type="text" id="recherche" placeholder="Rechercher un plat..." oninput="rechercherPlat()">
    </section>
    ...
    <nav class="filters">
        ...
        <label for="defaut">Par défaut</label>
            ...
            <?php foreach ($plats as $plat):?>
                <?php if ($plat["categorie"] === "plat"):?>
                    <article>
                        ...
                        <p><?= htmlspecialchars($plat["calories"]) ?> kcal</p>
                        <p>Allergènes: <?= htmlspecialchars($plat["allergenes"]) ?></p>
                        <span class="prix"><?= htmlspecialchars($plat["prix"]) ?>€</span>
                    </article>
                <?php endif; ?>
            <?php endforeach; ?>
        </section>
...
function chargerPlats() {
    const url = "cartefiltre.php?categorie=" + categorieActuelle + "&tag=" + tagActuel + "&tri=" + triActuel;
...
function afficherPlats(plats) {
    ...
    plats.forEach(plat => {
        // CORRECTION ICI : Utilisation des backticks (`) pour les littéraux de gabarits JavaScript
        html += `
            <article>
                <h2>${plat.nom}</h2>
