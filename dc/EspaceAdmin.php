...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($pseudo === $_SESSION['pseudo']) {
        $erreur = "Vous ne pouvez pas modifier votre propre compte. On modifie pas sa propre réalité.";
        ...
        if ($action === 'suspendre_temp') {
            ...
            if (empty($raison)) {
                $erreur = "Une raison est obligatoire — même l'arbitraire se justifie.";
            } else {
        ...
        if ($action === 'suspendre_def') {
            $raison = trim($_POST['raison'] ?? '');
            if (empty($raison)) {
                $erreur = "Le bannissement définitif, c'est sérieux. Une raison, s'il te plaît.";
            } else {
        ...
        if ($action === 'supprimer') {
            ...
            $message = "Le compte « {$pseudo} » a été supprimé. Adieu.";
        }
    }
    ...
    header('Location: EspaceAdmin.php' . $param);
    exit;
}
...
<div class="saint-denis">
    ...
    <div class="nanterre">
        ...
        <div class="creteil">
            <div class="bobigny bobigny-green"><?= $total_actifs ?></div>
            <div class="evry">Comptes actifs</div>
        </div>
        <div class="creteil">
            <div class="bobigny bobigny-orange"><?= $total_vip ?></div>
            <div class="evry">Membres VIP</div>
        </div>
        <div class="creteil">
            <div class="bobigny bobigny-red"><?= $total_suspendus ?></div>
    ...
    <form method="GET" action="" class="antony">
        ...
        <?php if ($filtre): ?>
            <a href="EspaceAdmin.php">Tout afficher</a>
        <?php endif; ?>
    </form>
    ...
    <?php else: ?>
        ...
        <div class="pontoise" data-pseudo="<?= htmlspecialchars($u['pseudo']) ?>">
            ...
            <div class="courbevoie">
                ...
                <?php else: ?>
                    ...
                    <div class="section-blocage">
                        <h4>⚡ Blocage rapide</h4>
                        <p class="blocage-desc">Bloquer immédiatement cet utilisateur (sa session sera terminée)</p>
                        <?php if ($u['statut'] === 'actif'): ?>
                            <button class="btn-bloquer pantin aulnay" data-pseudo="<?= htmlspecialchars($u['pseudo']) ?>">🚫 Bloquer maintenant</button>
                        <?php else: ?>
                            <button class="btn-debloquer pantin vitry" data-pseudo="<?= htmlspecialchars($u['pseudo']) ?>">✅ Débloquer</button>
                        <?php endif; ?>
                    </div>
